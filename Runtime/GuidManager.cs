    ...
    public class GuidManager
    {
        ...
        private bool InternalAdd(GuidComponent guidComponent)
        {
            ...
            if (existingInfo.go != null && existingInfo.go != guidComponent.gameObject)
            {
                ...
                {
                    string guidComponentName =
                        guidToObjectMap[guid].go != null ? guidToObjectMap[guid].go.name : "NULL";
                    string otherComponentName = guidComponent != null ? guidComponent.name : "NULL";

                    Debug.Assert(false,
                        $"Guid Collision Detected between {guidComponentName} and {otherComponentName}.\nAssigning new Guid. Consider tracking runtime instances using a direct reference or other method.");
                }
                else
                {
                    ...
                    Debug.LogWarning(
                        $"Guid Collision Detected while creating {guidComponent.name}.\nAssigning new Guid.",
                        guidComponent);
                }
        ...
        private static void OnChangeScene()
        {
            SceneManager.sceneLoaded += OnSceneLoaded;
        }

        private static void OnSceneLoaded(Scene arg0, LoadSceneMode mode)
        {
            var guidComponents = Object.FindObjectsOfType<GuidComponent>(true);
            foreach (var guidComponent in guidComponents)
            {
                if (!guidComponent.gameObject.activeInHierarchy)
                    Add(guidComponent);
            }
        }
    }
